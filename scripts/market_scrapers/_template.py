"""Download @@MARKET_NAME@@ financial statements from stockanalysis.com.
...
Output goes NEXT TO THIS SCRIPT in @@OUT_NAME@@/, never into the folder you launched from -
...
MARKET = "@@MARKET@@"
LIST_URL = "@@LIST_URL@@"
QUOTE_PREFIX = "@@QUOTE_PREFIX@@"      # stockanalysis path for this exchange

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
# The REPO ROOT, not this script's folder. These scrapers wrote to
# scripts/market_scrapers/@@OUT_NAME@@ while `consolidate-fundamentals` reads
# <repo>/@@OUT_NAME@@ - two directories with the same name, one of them invisible to the
...
DATA_DIR = os.environ.get("AERP_DATA_DIR") or os.path.join(_REPO_ROOT, "@@OUT_NAME@@")
...
def run_session():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print(f"\n@@MARKET_NAME@@: {total} symbols, {total - len(symbols)} already "
