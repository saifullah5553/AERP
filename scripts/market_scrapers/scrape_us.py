"""Download US financial statements from stockanalysis.com.
...
Output goes NEXT TO THIS SCRIPT in us_data/, never into the folder you launched from -
...
QUOTE_PREFIX = "stocks"      # stockanalysis path for this exchange

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
# The REPO ROOT, not this script's folder. These scrapers wrote to
# scripts/market_scrapers/us_data while `consolidate-fundamentals` reads
# <repo>/us_data - two directories with the same name, one of them invisible to the
...
DATA_DIR = os.environ.get("AERP_DATA_DIR") or os.path.join(_REPO_ROOT, "us_data")
...
def run_session():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print(f"\nUS: {total} symbols, {total - len(symbols)} already "
