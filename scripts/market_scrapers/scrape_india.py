"""Download India (NSE) financial statements from stockanalysis.com.

Simple and self-contained. One browser, one page, one symbol at a time.

    pip install playwright pandas
    python -m playwright install chromium
    python scrape_india.py
...
QUOTE_PREFIX = "quote/nse"      # stockanalysis path for this exchange

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
# The REPO ROOT, not this script's folder. These scrapers wrote to
# scripts/market_scrapers/india_data while `consolidate-fundamentals` reads
# <repo>/india_data - two directories with the same name, one of them invisible to the
...
DATA_DIR = os.environ.get("AERP_DATA_DIR") or os.path.join(_REPO_ROOT, "india_data")
...
def symbol_to_slug(symbol):
    ...
    # NSE's "-" and "&" both become "_" (BAJAJ-AUTO -> BAJAJ_AUTO).
