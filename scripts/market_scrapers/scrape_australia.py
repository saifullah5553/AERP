"""Download Australia (ASX) financial statements from stockanalysis.com.

Simple and self-contained. One browser, one page, one symbol at a time.

    pip install playwright pandas
    python -m playwright install chromium
    python scrape_australia.py
...
Output goes NEXT TO THIS SCRIPT in australia_data/, never into the folder you launched from -
...
QUOTE_PREFIX = "quote/asx"      # stockanalysis path for this exchange

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
# The REPO ROOT, not this script's folder. These scrapers wrote to
# scripts/market_scrapers/australia_data while `consolidate-fundamentals` reads
# <repo>/australia_data - two directories with the same name, one of them invisible to the
...
DATA_DIR = os.environ.get("AERP_DATA_DIR") or os.path.join(_REPO_ROOT, "australia_data")
...
def run_session():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print(f"\nAustralia (ASX): {total} symbols, {total - len(symbols)} already "
